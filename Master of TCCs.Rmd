...
resultado_pvclust = pvclust(dados_omit_NA[geneName,], method.dist="cor", method.hclust = "average", parallel=TRUE,nboot=1000)
...
1) Plotar uma distribuição do desvio padrão // trocar
2) Ordenar pelos 1000 primeiros genes
3) Pela distribuição da variação
4) ConsensusCluster
5) Verificar número de réplicas do bootstrap entre 100 ~ 500 ~ 1000
6) Aferir se o método de distância é correlação
7) Aferir se o método de cluster é average
8) Verificar se os gráficos estão corretos
9) PCA com todos os genes // mais informativo
10) Subtrai autismo e controle
11) ComplexHeatmap ler documentação
12) Colocar labels heatmap
13) Fazer histograma
14) Colocar histograma no TCC
...
17) Falar dos dados, idade, sexo, fenótipo e quadro clínico
